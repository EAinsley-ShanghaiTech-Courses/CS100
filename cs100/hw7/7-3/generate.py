...
try:
    point_num = int(
        input("- Please input number of points (100,000 ~ 800,000) "
              "[default = 300,000]: "))
except ValueError:
    point_num = 300000
try:
    cluster_num = int(
        input("- Please input number of theoretical clusters (e.g., centers) "
              "(1 ~ 20) [default = 15]: "))
except ValueError:
    cluster_num = 15
...
data, clist, centers = make_blobs(n_samples=point_num,
                                  centers=cluster_num,
                                  cluster_std=std,
                                  center_box=(-box, box),
                                  return_centers=True,
